/*
 * Unaligned templated vector type as standard implementation.
 *
...
#pragma endregion
    ...
    /// Standard implementation of an unaligned templated vector
    template <ASizeT Size, typename T>
    struct UTVector
    {
        // Metas
...
#pragma region Swizzle helpers
    ...
    {
        // "v.component<X>()" is not necessarily "x()"!
    ...
    {
        return UTVector<VectorSizes::Y, T>(
            BoolSelector<T, SelectX>(x(b), x(a)),
            BoolSelector<T, SelectY>(y(b), y(a)));
    }
    ...
    {
        return UTVector<VectorSizes::W, T>(
            BoolSelector<T, SelectX>(x(b), x(a)),
            BoolSelector<T, SelectY>(y(b), y(a)),
            BoolSelector<T, SelectZ>(z(b), z(a)),
            BoolSelector<T, SelectW>(w(b), w(a)));
    }
...
#pragma region Special math operations
    ...
    inline const typename UTVector<Size, T>::BoolType isNegative(const UTVector<Size, T> & v)
    {
        typedef typename UTVector<Size, T>::BoolType B;
        B t = Zero<B>();
        for (ASizeT i = Size; i > VectorIndices::X;)
    ...
    {
        struct H { static const T sqr(const T & v) { return v * v; } };
        return sqrt(H::sqr(x(a) - x(b)) + H::sqr(y(a) - y(b)) + H::sqr(z(a) - z(b)));
    }
    ...
    {
        T accumulator = v.x();
        for (ASizeT i = VectorIndices::Y; i < Size; ++i)
    ...
    {
        UTVector<Size, T> t;
        for (ASizeT i = VectorIndices::X; i < VectorSizes::Z; ++i)
            t.components[i] = exp(v.components[i]);
        for (ASizeT i = VectorIndices::W; i < Size; ++i)
            t.components[i] = v.components[i];
        return t;
