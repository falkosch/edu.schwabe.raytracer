...
namespace raytracer
{
        ...
        const SceneIntersection & intersection) const
    {
        return sampleLighting(
            ...
            containment.shininess,
            intersection,
            *containment.shadowCache,
    ...
    const LightShading SceneShader::sampleLighting(
        ...
        const Float4 & adaptedVisibilityCutoffIn,
        ...
        StatisticsCookie & statistics) const
    {
        ...
        Raycast shadowRay = Raycast(
            ...
            incidentRay.raycast.cullingMask,
        ...
        for (LightsCollection::const_iterator it = lights.cbegin(); it != lights.cend(); ++it)
        {
            ...
            if (isNegative(dotv(normalizedLightDirection, intersection.surfaceNormal))) continue;
			...
			const Float4 litAreaFraction = computeLitAreaFraction(
                ...
                shadowCache,
                statistics);
            if (allTrue(litAreaFraction < adaptedVisibilityCutoffIn)) continue;
#endif

            // Diffuse and specular reflection inbound intensity of this light source
            lighting.diffuse += litAreaFraction * attenuatedDiffuseIntensity * light.emissivity;
            lighting.specular += litAreaFraction * phongSpecularIntensityPerReflectedIncident(
                intersection.reflectedDirection,
