...
namespace raytracer
{
    ...
    void Raytracer::trigger(const RaytraceParameters & parameters)
    {
        ...
        const Size2 samplingResolution = max(One<Size2>(), convert<Size2>(convert<Float4>(parameters.resolution) * parameters.samplingFactor));
    ...
    inline const Float4 perPixelTiming(const UInt_64 start)
    {
        LARGE_INTEGER stop;
        QueryPerformanceCounter(&stop);
        return Float4(vectorization::log(
            static_cast<Float4::ValueType>(stop.QuadPart - start + One<UInt_64>())));
    ...
    {
        typedef std::vector<PackedRaytrace, vectorization::AlignedAllocator<PackedRaytrace>> ListType;
        Raytrace raytrace;
        Float4 * outputPixel;
    ...
    inline const PackedRaytrace::ListType constructPackedRaytracesList(const int packetIndex, const RaytracerPackets & packets, const RaytraceConfiguration & configuration)
    {
        ...
        for (Size2::ValueType cp = Zero<Size2::ValueType>(); cp < pixelsCount;)
        {
            ...
            Float4 * const outputPixel = &configuration.image->getData()[packets.imageIndexOfPixel(
                cpXY,
                configuration.image->getResolution())];
            for (Size2::ValueType s = Zero<Size2::ValueType>(); s < subsamplesCount;)
            {
                // interpolate the camera-point's top-left on the view-plane
                list.push_back(PackedRaytrace(Raytrace(Raycast(
