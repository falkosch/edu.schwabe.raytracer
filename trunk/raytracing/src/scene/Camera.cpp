...
namespace raytracer
{
    ...
    Camera::Camera()
        ...
        viewMatrix(Zero<Float44>()),
        worldPosition(),
        vfNearTopLeft(),
        vfNearRightDirection(),
    ...
    void Camera::translate(const Float3 & translationIn)
    {
        ...
        this->viewMatrix = transpose(replaceRow<VectorIndices::W>(
            transViewMatrix,
            transViewMatrix.row3 + convert<Float4>(translationIn)));
        updateView();
    }
    ...
    void Camera::rotate(const Float3 & rotationIn)
    {
        const Float44 rotateMatrix = vectorization::rotate(vectorization::rotate(vectorization::rotate(
            ...
            y(rotationIn), OneX<Float4>()), // Switch of X and Y rotation vectors is okay
    ...
    void Camera::updateView()
    {
        ...
        this->vfNearRightDirection = unproject(
            x_yzw(viewportExtends, NearZ), tInverseViewProjectionMatrix, viewport) - nearTopLeft;
        this->vfNearBottomDirection = unproject(
            blend<true, false, true, true>(viewportExtends, NearZ), tInverseViewProjectionMatrix, viewport) - nearTopLeft;
        this->vfFarRightDirection = unproject(
            x_yzw(viewportExtends, FarZ), tInverseViewProjectionMatrix, viewport) - farTopLeft;
