...
namespace raytracer
{
        ...
        fPermutationsCount(static_cast<Float>(DefaultPermutationsCount))
    {
        ...
        for (ASizeT i = Zero<ASizeT>(); i < permutationsCount; ++i)
        {
            ...
            // for multi-dimensional gradients, the gradients are normalized to still nicely fit into the [-1,1] range
    ...
    const Float PerlinNoiseGenerator::noise(const Float v) const
    {
        ...
        Float4 rx;
        setup(v, bx, rx);
        const Float4 uv = rx * Float4(
            x(gradients1[permutations[x(bx)]]),
            x(gradients1[permutations[y(bx)]]));
    ...
    const Float PerlinNoiseGenerator::noise2(const Float4 & v) const
    {
        ...
        const Float a = mix(
            dot3(Float4(x(rx), x(ry)), gradients2[x(b0)]),
            dot3(Float4(y(rx), x(ry)), gradients2[x(b1)]),
            x(s));
        const Float b = mix(
            dot3(Float4(x(rx), y(ry)), gradients2[y(b0)]),
    ...
    const Float PerlinNoiseGenerator::noise3(const Float4 & v) const
    {
        ...
        const Float a = mix(
            dot3(Float4(x(rx), x(ry), x(rz)), gradients3[x(b0) + x(bz)]),
            dot3(Float4(y(rx), x(ry), x(rz)), gradients3[x(b1) + x(bz)]),
            x(s));
        const Float b = mix(
            dot3(Float4(x(rx), y(ry), x(rz)), gradients3[y(b0) + x(bz)]),
            dot3(Float4(y(rx), y(ry), x(rz)), gradients3[y(b1) + x(bz)]),
            x(s));
        const Float c = mix(
            dot3(Float4(x(rx), x(ry), y(rz)), gradients3[x(b0) + y(bz)]),
            dot3(Float4(y(rx), x(ry), y(rz)), gradients3[x(b1) + y(bz)]),
            x(s));
        const Float d = mix(
            dot3(Float4(x(rx), y(ry), y(rz)), gradients3[y(b0) + y(bz)]),
            dot3(Float4(y(rx), y(ry), y(rz)), gradients3[y(b1) + y(bz)]),
            x(s));
    ...
    const Float PerlinNoiseGenerator::noise4(const Float4 &v) const
    {
        ...
        const Float a = mix(
            dot(Float4(x(rx), x(ry), x(rz), One<Float>()), gradients4[x(b0) + x(bz)]),
            dot(Float4(y(rx), x(ry), x(rz), One<Float>()), gradients4[x(b1) + x(bz)]),
            x(s));
        const Float b = mix(
            dot(Float4(x(rx), y(ry), x(rz), One<Float>()), gradients4[y(b0) + x(bz)]),
            dot(Float4(y(rx), y(ry), x(rz), One<Float>()), gradients4[y(b1) + x(bz)]),
            x(s));
        const Float c = mix(
            dot(Float4(x(rx), x(ry), y(rz), One<Float>()), gradients4[x(b0) + y(bz)]),
            dot(Float4(y(rx), x(ry), y(rz), One<Float>()), gradients4[x(b1) + y(bz)]),
            x(s));
        const Float d = mix(
            dot(Float4(x(rx), y(ry), y(rz), One<Float>()), gradients4[y(b0) + y(bz)]),
            dot(Float4(y(rx), y(ry), y(rz), One<Float>()), gradients4[y(b1) + y(bz)]),
            x(s));
        return mix(mix(a, b, y(s)), mix(c, d, y(s)), z(s));
    }
