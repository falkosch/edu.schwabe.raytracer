...
namespace raytracer
{
    ...
    const BITMAPINFO Bitmap::getBITMAPINFO() const
    {
        ...
        sysBitmapInfo.bmiHeader.biBitCount
            = static_cast<WORD>((VectorType::SIZE * sizeof(VectorType::ValueType)) << static_cast<ASizeT>(3));
        ...
        sysBitmapInfo.bmiHeader.biXPelsPerMeter = 0x0ec4;
    ...
    Bitmap * const Bitmap::loadPPM(const std::string & filename)
    {
        ...
        do getline(file, dimensions);
        while (dimensions[Zero<ASizeT>()] == '#');

        Size2 resolution;
        std::stringstream(dimensions)
            >> resolution[VectorIndices::X] >> resolution[VectorIndices::Y];
        ...
        const Int MIN = static_cast<Int>(VectorType::ValueLimits::lowest());
        const Int MAX = static_cast<Int>(VectorType::ValueLimits::max());
        ...
        for (int i = Zero<int>(); i < heighti; ++i)
        {
            ...
            for (ASizeT j = VectorIndices::X; j < rawStride; j += VectorType::SIZE)
            {
                ...
                const Int4 texel = (Int4(
                    static_cast<VectorType::ValueType>(*(rawData)),
                    static_cast<VectorType::ValueType>(*(rawData + 1)),
                    static_cast<VectorType::ValueType>(*(rawData + 2))) + VOFFSET) * VSCALE;

                VectorType::ValueType * dataLoaded = &(loaded->data[scanline + j]);
                *(dataLoaded++) = static_cast<VectorType::ValueType>(x(texel));
                *(dataLoaded++) = static_cast<VectorType::ValueType>(y(texel));
                *(dataLoaded) = static_cast<VectorType::ValueType>(z(texel));
            }
        }
