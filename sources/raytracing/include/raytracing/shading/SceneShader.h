...
namespace raytracer {
  ...
  public:
    ...
    // Samples the lighting of a facet in the scene.
    const LightShading sample(const SceneShaderContainment &containment, const SceneIntersection &intersection) const;
  ...
  private:
    ...
    // at the facet.
    static const Float4 lambertDiffuseIntensity(const Float4 &lightDirection, const Float4 &normal);

    // Calculates the attenuated diffuse intensity. Vector attenuationFactors contains the light attenuation
    // over distance in empty space based parameters with z-component as quadratic (Inverse-square-law; generally
    // for ideal point lights), y-component as linear (for spotlights) and x-component as constant (planar lights)
    // falloff factor.
