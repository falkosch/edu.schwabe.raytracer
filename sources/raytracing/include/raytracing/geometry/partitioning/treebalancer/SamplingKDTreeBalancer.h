...
namespace raytracer
{
    ...
    {
    public:
        ~SamplingKDTreeBalancer() override;

    protected:
        static const AxisAlignedBoundingBox geometryNodeBox(const GeometryNode& node);

        static const KDTreePlane
        sampleSplittingPlane(ASizeT axis, const AxisAlignedBoundingBox& bounding, const Float4& sample);

        static const KDTreePlane
        sampleSplittingPlane(ASizeT axis, const Float4& offset, const Float4& normalization, const Float4& sample);

        static const KDTreePlane
        sampleSplittingPlane(ASizeT axis, Float offset, Float normalization, Float sample);
        ...
        static const PGeometryNodeList geometrySortByMinimum(ASizeT axis, const PGeometryNodeList& geometry);

        static const PGeometryNodeList geometrySortByMaximum(ASizeT axis, const PGeometryNodeList& geometry);
