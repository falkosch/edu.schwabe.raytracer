...
namespace raytracer {
  ...
  HDRImage::HDRImage(const Bitmap &bitmap) : resolution(bitmap.getResolution()), resolutionf(), data() {
...
#pragma omp parallel for
  ...
  const Float4 HDRImage::sampleBilinear(const Float4 &texCoords) const {
    ...
    return mix(
        mix(data[x(TL_TR)], data[y(TL_TR)], xxxx(partialArea)), mix(data[x(BL_BR)], data[y(BL_BR)], xxxx(partialArea)),
        yyyy(partialArea)
    );
  }
}
