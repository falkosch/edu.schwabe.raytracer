...
namespace raytracer
{
    ...
    Mesh* Resources::getMesh(const std::string& identifier)
    {
        ...
        if (found == meshes.end())
        {
            ...
            if (found == meshes.end())
            {
                ...
                auto newMesh = Mesh::loadFromOffFile(file, hasRevertedNormals(file), std::move(traverser),
                                                     std::move(balancer));
    ...
    HDRImage* Resources::loadPPM(const std::string& identifier, const std::string& cacheKey,
                                 const bool srgbColorConversion)
    {
        auto found = textures.find(cacheKey);
        if (found == textures.end())
        {
            ...
            if (found == textures.end())
            {
                ...
                auto image = std::make_unique<HDRImage>(*loadedBitmap, srgbColorConversion);
                auto* ptr = image.get();
                textures[file != cacheKey ? cacheKey : file] = std::move(image);
                return ptr;
            }
        }
        return found->second.get();
    ...
    {
        return loadPPM(identifier, identifier, true);
    }

    HDRImage* Resources::getLinearPPM(const std::string& identifier)
    {
        return loadPPM(identifier, "linear:" + identifier, false);
    }
}
