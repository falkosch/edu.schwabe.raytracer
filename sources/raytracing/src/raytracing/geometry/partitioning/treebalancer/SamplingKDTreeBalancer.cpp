...
namespace raytracer {
  SamplingKDTreeBalancer::~SamplingKDTreeBalancer() = default;
  ...
  SamplingKDTreeBalancer::geometrySortByMinimum(const ASizeT axis, const PGeometryNodeList &geometry) {
    switch (axis) {
  ...
  const PGeometryNodeList
  SamplingKDTreeBalancer::geometrySortByMaximum(const ASizeT axis, const PGeometryNodeList &geometry) {
    switch (axis) {
    case VectorIndices::X:
  ...
  const KDTreePlane SamplingKDTreeBalancer::sampleSplittingPlane(
      const ASizeT axis, const AxisAlignedBoundingBox &bounding, const Float4 &sample
  ) {
    return sampleSplittingPlane(axis, bounding.minimum, extents(bounding), sample);
  }

  const KDTreePlane SamplingKDTreeBalancer::sampleSplittingPlane(
      const ASizeT axis, const Float4 &offset, const Float4 &normalization, const Float4 &sample
  ) {
    return sampleSplittingPlane(axis, offset[axis], normalization[axis], sample[axis]);
  }

  const KDTreePlane SamplingKDTreeBalancer::sampleSplittingPlane(
      const ASizeT axis, const Float offset, const Float normalization, const Float sample
  ) {
