...
#pragma omp parallel for
        for (auto i = int{ 0 }; i < static_cast<int>(facets.size()); i++) {
            ...
            planeNormals[index] = FacetNormals{
                replaceW(planeNormal0, -dot3(facet.v0, planeNormal0)),
                replaceW(planeNormal1, -dot3(facet.v0, planeNormal1)),
            ...
            const FacetNormals normalsWeighted{
                flatNormal / (l0 * l2),
                flatNormal / (l1 * l0),
                flatNormal / (l2 * l1)
            };
...
#pragma omp parallel for
        for (auto i = int{ 0 }; i < static_cast<int>(facets.size()); i++) {
            ...
            smoothNormals[index] = FacetNormals{
                select(
                    dot3(flatNormal, normalV0) < Sin45<Float>(),
                    flatNormal,
                    normalV0
                ),
                select(
                    dot3(flatNormal, normalV1) < Sin45<Float>(),
                    flatNormal,
                    normalV1
                ),
                select(
                    dot3(flatNormal, normalV2) < Sin45<Float>(),
                    flatNormal,
                    normalV2
                )
            };
...
#pragma omp parallel for
    ...
    const Float computeFacetIntersection(
        const ASizeT index,
        const Float4 & uvAndDistance,
        const Raycast & raycast,
        const std::vector<FacetTexCoords4> & texCoords,
        ...
        intersectionOut.smoothedNormal = normalize3(
            baryCenter(uvAndDistance, smoothNormals[index])
        );
        intersectionOut.texCoords = xy_zw(
            baryCenter(uvAndDistance, texCoords[index]),
            OneW<Float4>()
        );
