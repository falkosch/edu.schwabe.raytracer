...
namespace raytracer
{
  ...
  bool triBoxOverlap(const AxisAlignedBoundingBox& aabb, const Facet& trianglePlanes)
  {
    ...
    if (const auto triBox = bounding(trianglePlanes); !overlaps(aabb, triBox))
    {
      return false;
    }
    ...
    // the triangle
    const auto f0 = trianglePlanes.v1 - trianglePlanes.v0;
    const auto f1 = trianglePlanes.v2 - trianglePlanes.v1;
    const auto n = cross3(f0, f1);
    const auto boxHalfSize = halfLengths(aabb);
    const auto boxCenter = center(aabb, boxHalfSize);
    const auto v0 = trianglePlanes.v0 - boxCenter;
    ...
    const auto v1 = trianglePlanes.v1 - boxCenter;
    const auto v2 = trianglePlanes.v2 - boxCenter;
    const auto yzxV0 = yzxw(v0);
    const auto zxyV0 = zxyw(v0);
    const auto yyyV1 = yyyy(v1);
    const auto yzxV2 = yzxw(v2);
    const auto zxyV2 = zxyw(v2);
    const auto yzxBoxHalfSize = yzxw(boxHalfSize);
    const auto zxyBoxHalfSize = zxyw(boxHalfSize);
