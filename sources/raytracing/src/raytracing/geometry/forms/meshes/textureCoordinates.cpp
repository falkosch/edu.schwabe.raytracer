...
namespace raytracer {
  void computeTexCoordsOrtho(
      const std::vector<UInt3> &facetIndices, const std::vector<Facet> &facets, const Float4 &sPlane,
      const Float4 &tPlane, std::vector<FacetTexCoords4> &texCoords
  ) noexcept {
...
#pragma omp parallel for
  ...
  void computeTexCoordsSpherical(
      const std::vector<UInt3> &facetIndices, const std::vector<Facet> &facets, std::vector<FacetTexCoords4> &texCoords
  ) noexcept {
    texCoords.resize(facetIndices.size());
...
#pragma omp parallel for
    for (auto i = int{0}; i < static_cast<int>(facetIndices.size()); i++) {
      ...
      // normalize vertices, they are used as normals for determining the tex-coords
      // "- Common::ONE_W" eliminates w-coord, which is always 1.0f for vertices
      auto &facet = facets[index];
      const Facet normalized{normalize3(facet.v0), normalize3(facet.v1), normalize3(facet.v2)};
      ...
      // so determine min, mid and max to check where a triangle is positioned.
      if (z(facet.v0) > 0.0f || z(facet.v1) > 0.0f || z(facet.v2) > 0.0f)
        continue;

      // search min, max, and mid of facet
      auto min = facet.v0;
      auto mid = facet.v0;
      auto max = facet.v0;
      ...
      if (x(min) < 0.0f && x(max) > 0.0f) {
        // case 1.1 for easier fixing: if mid->x is not negative, only minUV needs to be fixed
        if (x(mid) >= 0.0f)
          minUV += wzyx(OneW<Float4>());
        else // case 1.2: mid is on the negative side, only maxUV needs to be fixed
