...
namespace vectorization {
  ...
  inline PackedFloat2_128 swizzledBlend(const PackedFloat2_128 &a, const PackedFloat2_128 &b) noexcept {
    return blend<SelectX, SelectY>(swizzle<X, Y>(a), swizzle<X, Y>(b));
  }

  // x(a), x(a)
  template <>
  PackedFloat2_128 swizzledBlend<VectorIndices::X, VectorIndices::X, false, false>(
  ...
  // y(a), x(a)
  template <>
  PackedFloat2_128 swizzledBlend<VectorIndices::Y, VectorIndices::X, false, false>(
      const PackedFloat2_128 &a, const PackedFloat2_128 &b
  ) noexcept;
