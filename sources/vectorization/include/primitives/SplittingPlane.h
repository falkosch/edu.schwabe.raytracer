...
namespace primitives {
  ...
  const bool overlaps(const RayCast &rayCast, const SplittingPlane &by) noexcept;

  const Float
  nearestIntersection(const RayCast &rayCast, const SplittingPlane &p, const Size2::ValueType originId) noexcept;
}
