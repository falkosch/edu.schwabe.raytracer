...
namespace primitives
{
    ...
    const AxisAlignedBoundingBox extendBy(const AxisAlignedBoundingBox & a, const Float4 & p) noexcept {
    ...
    const AxisAlignedBoundingBox extendBy(const AxisAlignedBoundingBox & a, const Float4 & minimumIn, const Float4 & maximumIn) noexcept {
        AxisAlignedBoundingBox r;
    ...
    const AxisAlignedBoundingBox extendBy(const AxisAlignedBoundingBox & a, const AxisAlignedBoundingBox & b) noexcept {
        return extendBy(a, b.minimum, b.maximum);
    }

    const AxisAlignedBoundingBox clamp(const AxisAlignedBoundingBox & a, const AxisAlignedBoundingBox & b) noexcept {
        AxisAlignedBoundingBox r;
    ...
    {
        static void apply(AxisAlignedBoundingBox & out, const AxisAlignedBoundingBox & toTransform, const Float44 & m) noexcept {
            const Float4 f = row<RowIndex>(m);
            const Float4 a = f * toTransform.minimum, b = f * toTransform.maximum;
            out.minimum = replaceComponent<RowIndex>(out.minimum, component<RowIndex>(out.minimum + horizontalSum3v(min(a, b))));
            out.maximum = replaceComponent<RowIndex>(out.maximum, component<RowIndex>(out.maximum + horizontalSum3v(max(a, b))));
        }
    };

    const AxisAlignedBoundingBox transform(const AxisAlignedBoundingBox & a, const Float44 & m) noexcept {
    ...
    const Float4::VectorBoolType overlaps(const Raycast & r, const AxisAlignedBoundingBox & a, const AxisAlignedBoundingBox & b) noexcept {
    ...
    inline const Float4 computeBoxIntersectionCoefficients(const Ray & r, const AxisAlignedBoundingBox & a) noexcept {
        ...
        return xz_xz(
            xy_zw(
                ...
                min(hMinXXYZx2, yyww(hMinXXYZx2))
            ),
            Zero<Float4>()
        );
    }
    ...
    // z = t0 < 0 = c.x < c.z
    // w = t1 < 0 = c.y < c.z
    inline const Float4::VectorBoolType testBoxIntersectionCoefficients(const Float4 & c) noexcept {
        return xyxy(c) < xxzz(c);
    }

    const bool overlaps(const Raycast & r, const AxisAlignedBoundingBox & a) noexcept {
    ...
    const Float nearestIntersection(const Raycast & r, const AxisAlignedBoundingBox & a, const Size2::ValueType originId) noexcept {
        const Float4 c = computeBoxIntersectionCoefficients(r.ray, a);
        const Float4::VectorBoolType check = testBoxIntersectionCoefficients(c);
