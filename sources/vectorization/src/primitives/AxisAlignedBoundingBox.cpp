...
namespace primitives {
  ...
  extendBy(const AxisAlignedBoundingBox &a, const Float4 &minimumIn, const Float4 &maximumIn) noexcept {
    AxisAlignedBoundingBox r;
  ...
  const AxisAlignedBoundingBox transform(const AxisAlignedBoundingBox &a, const Float44 &m) noexcept {
    ...
    const Float4 t = yw_yw(
        zw_zw(row<VectorIndices::X>(m), row<VectorIndices::Y>(m)), zw_zw(row<VectorIndices::Z>(m), One<Float4>())
    );
  ...
  // The near distance of "a" is set to "t.x". The far distance of "a" is set "t.y". Vice versa is set for "b" as "t.z"
  // and "t.w".
  inline const Float4 computeBoxIntersectionCoefficients(
      const Ray &r, const AxisAlignedBoundingBox &a, const AxisAlignedBoundingBox &b
  ) noexcept {
  ...
  overlaps(const RayCast &r, const AxisAlignedBoundingBox &a, const AxisAlignedBoundingBox &b) noexcept {
    ...
    return (cYW >= cXZ) & andnot(andnot(cXZ < Zero<Float4>(), outOfReach(r, cXZ)), (cYW >= Zero<Float4>()));
  }
  ...
  const Float
  nearestIntersection(const RayCast &r, const AxisAlignedBoundingBox &a, const Size2::ValueType originId) noexcept {
    const Float4 c = computeBoxIntersectionCoefficients(r.ray, a);
    const Float4::VectorBoolType check = testBoxIntersectionCoefficients(c);
