...
namespace vectorization {
...
#ifdef ARCH_X64
    return static_cast<v_i64_2::ValueType>(_mm_cvtsi128_si64(vector.components));
#else
    std::array<v_i64_2::ValueType, v_i64_2::SIZE> components;
    std::memcpy(components.data(), &vector.components, sizeof(v_i64_2::PackedType));
...
#ifdef ARCH_X64
    return static_cast<v_i64_2::ValueType>(_mm_extract_epi64(vector.components, VectorIndices::Y));
#else
    std::array<v_i64_2::ValueType, v_i64_2::SIZE> components;
    std::memcpy(components.data(), &vector.components, sizeof(v_i64_2::PackedType));
  ...
  template <>
  v_i64_2 replaceComponent<VectorIndices::X>(const v_i64_2 &vector, const v_i64_2::ValueType scalar) noexcept {
#ifdef ARCH_X64
    return _mm_insert_epi64(vector.components, static_cast<long long>(scalar), VectorIndices::X);
#else
    return v_i64_2(scalar, y(vector));
#endif
  }

  template <>
  v_i64_2 replaceComponent<VectorIndices::Y>(const v_i64_2 &vector, const v_i64_2::ValueType scalar) noexcept {
#ifdef ARCH_X64
    return _mm_insert_epi64(vector.components, static_cast<long long>(scalar), VectorIndices::Y);
#else
    return v_i64_2(x(vector), scalar);
#endif
  }

  v_i64_2 replaceX(const v_i64_2 &vector, const v_i64_2::ValueType scalar) noexcept {
    return replaceComponent<VectorIndices::X>(vector, scalar);
  }

  v_i64_2 replaceY(const v_i64_2 &vector, const v_i64_2::ValueType scalar) noexcept {
