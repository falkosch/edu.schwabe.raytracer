...
namespace vectorization {
  ...
  const PackedFloat4_128 sign(const PackedFloat4_128 &v) noexcept {
    return blendMasked(
        copySign(v), Zero<PackedFloat4_128>(),
        _mm_cmpeq_ps(Zero<PackedFloat4_128>(), _mm_andnot_ps(NegativeZero<PackedFloat4_128>(), v))
    );
  }

  const PackedFloat2_128 sign(const PackedFloat2_128 &v) noexcept {
    return blendMasked(
        copySign(v), Zero<PackedFloat2_128>(),
        _mm_cmpeq_pd(Zero<PackedFloat2_128>(), _mm_andnot_pd(NegativeZero<PackedFloat2_128>(), v))
    );
  }
