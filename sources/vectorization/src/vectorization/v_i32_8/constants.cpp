...
namespace vectorization {
  ...
  v_i32_8 One<v_i32_8>() noexcept {
    ...
    return _mm256_srli_epi32(
        MaskAll<PackedInts_256>(), sizeof(v_i32_8::ValueType) * std::numeric_limits<unsigned char>::digits - 1
    );
...
#if VECTORIZATION_INTRINSICS_LEVEL >= VECTORIZATION_AVX
    return MaskAll<PackedInts_256>();
#else
    const auto allOnes = MaskAll<v_i32_8::HalfPackedType>();
...
#if VECTORIZATION_INTRINSICS_LEVEL >= VECTORIZATION_AVX2
    return _mm256_slli_epi32(MaskAll<PackedInts_256>(), 1);
#elif VECTORIZATION_INTRINSICS_LEVEL >= VECTORIZATION_AVX
    return _mm256_set1_epi32(-2);
#else
    const auto negTwo = _mm_set1_epi32(-2);
    return {{negTwo, negTwo}};
#endif
  }
}
