...
namespace raytracerui
{
    ...
    void OpenGLWindowsRaytracerUI::reshape(const Size2& newSize)
    {
        WindowsRaytracerUI::reshape(newSize);
        glViewport(0, 0, static_cast<GLsizei>(x(newSize)), static_cast<GLsizei>(y(newSize)));
    }
    ...
    void OpenGLWindowsRaytracerUI::renderUI()
    {
        ...
        if (showMetrics)
        {
            ...
            if (ImGui::Begin("Metrics (F2)", &showMetrics))
            {
                ...
                const auto totalRays = static_cast<double>(s.primaryRays + s.secondaryRays + s.shadowRays + s.
                    objectShadowRays);
                if (lastRenderDuration > 0.0)
                {
                    ImGui::Text("Duration: %.4f s  (%.2f MRays/s)", lastRenderDuration,
                                totalRays / lastRenderDuration / 1e6);
                }
                else
                {
        ...
        if (showConfig)
        {
            ...
            if (ImGui::Begin("Configuration (F3)", &showConfig))
            {
                ...
                const char* sceneItems[] = {
                    "Cornell Box", "Test Scene 1", "Test Scene 2", "Dragon", "Procedural"
                };
                if (ImGui::Combo("Scene", &sceneIndex, sceneItems, 5))
                {
                    const auto idx = sceneIndex;
                    auto* s = scene;
                    auto* r = resources;
                    raytracer->enqueueWork([s, r, idx]
                    {
                        s->getSceneObjects().clear();
                        s->getLights().clear();
                        s->setBackgroundShader(nullptr);
                        s->setAmbientLight(RGBS::black());
                        switch (idx)
                        {
                        case 0: CornellBoxScene::setup(*s, *r);
                            break;
                        case 1: TestScene1::setup(*s, *r);
                            break;
                        case 2: TestScene2::setup(*s, *r);
                            break;
                        case 3: DragonScene::setup(*s, *r);
                            break;
                        case 4: ProceduralScene<>::setup(*s, *r);
                            break;
                        default: break;
                        }
                        s->buildSceneGraph();
                ...
                if (meshBalancerChanged && scene)
                {
                    auto* s = scene;
                    const auto idx = meshBalancerIndex;
                    const auto thr = meshBalancerThreshold;
                    raytracer->enqueueWork([s, idx, thr]
                    {
                        s->setMeshTreeBalancer([idx, thr] { return createBalancer(idx, thr); });
                    });
                    changed = true;
                }
                ...
                if (output && ImGui::Button("Save As..."))
                {
                    ...
                    if (GetSaveFileNameA(&ofn))
                    {
                        ...
                        const auto res = (ext == "png") ? output->saveAsPNG(path) : output->saveAsBMP(path);
                        Log.info([path, res] { return "Saved image to " + path + "; result: " + std::to_string(res); });
                    }
                }
