...
namespace primitives
{
    ...
    {
        static void
        apply(AxisAlignedBoundingBox& out, const AxisAlignedBoundingBox& toTransform, const MFloat4x4& m) noexcept
        {
            const auto f = row<RowIndex>(m);
            const auto a = f * toTransform.minimum, b = f * toTransform.maximum;
            const auto minDistanceSum = horizontalSum3v(min(a, b));
            const auto maxDistanceSum = horizontalSum3v(max(a, b));
            out.minimum = replaceComponent<RowIndex>(out.minimum, component<RowIndex>(out.minimum + minDistanceSum));
            out.maximum = replaceComponent<RowIndex>(out.maximum, component<RowIndex>(out.maximum + maxDistanceSum));
        }
    };

    AxisAlignedBoundingBox transform(const AxisAlignedBoundingBox& box, const MFloat4x4& by) noexcept
    {
