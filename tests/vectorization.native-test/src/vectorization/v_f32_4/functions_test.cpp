...
namespace vectorization::test
{
    TEST_CLASS (v_f32_4_FunctionsTest)
    {
        ...
        TEST_METHOD(computesIsNegative)
        {
            ...
            Assert::AreEqual(static_cast<v_f32_4::BoolType>(VectorBits::X | VectorBits::Z), mask,
                             L"isNegative mismatch",
                             LINE_INFO());

            // all positive
            const auto maskPos = isNegative(v_f32_4{1.0f, 2.0f, 3.0f, 4.0f});
            Assert::AreEqual(static_cast<v_f32_4::BoolType>(VectorBits::None), maskPos,
                             L"isNegative all-positive mismatch",
                             LINE_INFO());
        ...
        {
            const v_f32_4 v{2.0f, 3.0f, 4.0f, 5.0f};
            const auto s = sqr(v);
            Assert::AreEqual(4.0f, x(s), L"sqr X mismatch", LINE_INFO());
            Assert::AreEqual(9.0f, y(s), L"sqr Y mismatch", LINE_INFO());
            Assert::AreEqual(16.0f, z(s), L"sqr Z mismatch", LINE_INFO());
            Assert::AreEqual(25.0f, w(s), L"sqr W mismatch", LINE_INFO());

            const v_f32_4 neg{-3.0f, 0.0f, -1.0f, 0.5f};
            const auto s2 = sqr(neg);
            Assert::AreEqual(9.0f, x(s2), L"sqr negative", LINE_INFO());
            Assert::AreEqual(0.0f, y(s2), L"sqr zero", LINE_INFO());
            Assert::AreEqual(1.0f, z(s2), L"sqr -1", LINE_INFO());
            Assert::AreEqual(0.25f, w(s2), L"sqr 0.5", LINE_INFO());
        }
