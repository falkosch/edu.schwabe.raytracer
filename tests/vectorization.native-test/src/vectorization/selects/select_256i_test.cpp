...
namespace vectorization::test {
  ...
  public:
    TEST_METHOD(selectsOneOfTwoPackedInts_128WithBoolConditionMask) {
      const auto givenTruthy = _mm256_set1_epi32(1);
      const auto givenFalsy = _mm256_set1_epi32(2);

      {
        const auto actual = select(true, givenTruthy, givenFalsy);
        const auto matchExpected = _mm256_cmpeq_epi32(givenTruthy, actual);
        Assert::AreEqual(
...

      {
        ...
        const auto matchExpected = _mm256_cmpeq_epi32(givenFalsy, actual);
        Assert::AreEqual(
            1, _mm256_testc_si256(matchExpected, MaskAll<PackedInts_256>()), L"select value mismatch", LINE_INFO()
        );
      }
