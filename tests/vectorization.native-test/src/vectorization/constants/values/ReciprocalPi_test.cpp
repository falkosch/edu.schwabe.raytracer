...
namespace vectorization::test {
  TEST_CLASS(ValueReciprocalPiEpsilonTest) {
  public:
    TEST_METHOD(returnsFloat_32) {
      Assert::AreNotEqual(0.0f, ReciprocalPi<Float_32>());
    }

    TEST_METHOD(returnsFloat_64) {
      Assert::AreNotEqual(0.0, ReciprocalPi<Float_64>());
    }

    TEST_METHOD(returnsPackedFloat2_128) {
      constexpr std::array notExpected{0.0, 0.0};
      std::array<Float_64, VectorSizes::Y> actual{};
      _mm_store_pd(actual.data(), ReciprocalPi<PackedFloat2_128>());
    ...
    TEST_METHOD(returnsPackedFloat4_128) {
      ...
      _mm_store_ps(actual.data(), ReciprocalPi<PackedFloat4_128>());
      Assert::AreNotEqual(notExpected, actual);
    }
    ...
    TEST_METHOD(returnsPackedFloat4_256) {
      ...
      _mm256_store_pd(actual.data(), ReciprocalPi<PackedFloat4_256>());
      Assert::AreNotEqual(notExpected, actual);
    }
    ...
    TEST_METHOD(returnsPackedFloat8_256) {
      ...
      _mm256_store_ps(actual.data(), ReciprocalPi<PackedFloat8_256>());
      Assert::AreNotEqual(notExpected, actual);
    }
  };
}
