...
namespace vectorization::test {
  TEST_CLASS(v_i64_2_AccessorsTest) {
  public:
    TEST_METHOD(readsComponent) {
      const v_i64_2 given{Int_64{10LL}, Int_64{-5LL}};

      {
        const v_i64_2 actual{component<VectorIndices::X>(given), component<VectorIndices::Y>(given)};
        Assert::IsTrue(allTrue(given == actual), L"component value mismatch", LINE_INFO());
      }

      {
        const v_i64_2 actual{component<VectorIndices::Y>(given), component<VectorIndices::X>(given)};
    ...
    TEST_METHOD(readsXY) {
      ...
      {
        const v_i64_2 actual{x(given), y(given)};
        Assert::IsTrue(allTrue(given == actual), L"component value mismatch", LINE_INFO());
      }

      {
        const v_i64_2 actual{y(given), x(given)};
        Assert::IsTrue(allTrue(given != actual), L"wrong component match", LINE_INFO());
      }
    }

    TEST_METHOD(replacesComponent) {
      const Int_64 givenReplacement = 30LL;
      const v_i64_2 given{Int_64{10LL}, Int_64{-5LL}};

      {
        const v_i64_2 expected{givenReplacement, y(given)};
      ...
      {
        const v_i64_2 expected{x(given), givenReplacement};
