...
namespace vectorization::test {
  ...
  public:
    TEST_METHOD(readsComponent) {
      ...
      {
        std::array<Float_64, VectorSizes::Y> actual{
            component<VectorIndices::Y>(givenPack), component<VectorIndices::X>(givenPack)
        };
    ...
    TEST_METHOD(readsXY) {
      ...
      {
        std::array<Float_64, VectorSizes::Y> actual{x(givenPack), y(givenPack)};
        Assert::AreEqual(expectedComponents, actual, L"component value mismatch", LINE_INFO());
      }

      {
        std::array<Float_64, VectorSizes::Y> actual{y(givenPack), x(givenPack)};
        Assert::AreNotEqual(expectedComponents, actual, L"wrong component match", LINE_INFO());
      }
    }
    ...
    TEST_METHOD(readsXi) {
      ...
      {
        std::array<Float_64, VectorSizes::Y> actual{x1(givenPack), x2(givenPack)};
        Assert::AreEqual(expectedComponents, actual, L"component value mismatch", LINE_INFO());
      }

      {
        std::array<Float_64, VectorSizes::Y> actual{x2(givenPack), x1(givenPack)};
        Assert::AreNotEqual(expectedComponents, actual, L"wrong component match", LINE_INFO());
      }
    }
  };
