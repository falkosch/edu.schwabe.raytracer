...
namespace vectorization::test {
  struct MessageFormat {
    ...
    static const std::wstring forFunction(
      ...
      auto messageStream = std::ostringstream{} << "[" << index << "] " << value << " " << prefix << ": " << actual
                                                << " " << operatorText << " " << expected;
      return MessageFormat::toString(messageStream);
    }

    static const std::wstring
    forIndex(const ASizeT index, std::function<std::ostream &(std::ostream &)> writeMessageToStream) {
    ...
    static const std::wstring forIterationValue(
        const ASizeT index, const T testValue, std::function<std::ostream &(std::ostream &)> writeMessageToStream
    ) {
